...
def construct_metadata_files(m, tr_nobjs, valtest_nobjs, output_dir=defaultDir, suffix=""):
    ...
    for i,c in enumerate(tr_nobjs.index.values):
        ...
        if val_test_quantity < 100:
            val_test_quantity = 100
...
def construct_data_files(output_dir=defaultDir, suffix=""):
    ...
    """
    train_m = load_meta_data(output_dir+"train_metadata"+suffix+".csv")
    print(train_m.shape)
    train_ids = train_m["object_id"].values
    print(train_ids.size)

    val_m = load_meta_data(output_dir+"val_metadata"+suffix+".csv")
    print(val_m.shape)
    val_ids = val_m["object_id"].values
    print(val_ids.size)

    test_m = load_meta_data(output_dir+"test_metadata"+suffix+".csv")
    print(test_m.shape)
    test_ids = test_m["object_id"].values
    print(test_ids.size)
    chunksize = 10**6
    #loop over plasticc test data to find objects for which I sampled ids
    for i,chunk in enumerate(pd.read_csv(output_dir+"plasticc_data.csv", chunksize = chunksize)):

        print("chunk number: "+str(i))
        print(chunk.shape)
        
        train_chunk = chunk[chunk.object_id.isin(train_ids)]
        print(train_chunk.shape)
        
        val_chunk = chunk[chunk.object_id.isin(val_ids)]
        print(val_chunk.shape)
        
        test_chunk = chunk[chunk.object_id.isin(test_ids)]
        print(test_chunk.shape)
        
        if i == 0:
            train_chunk.to_csv(output_dir+"train_data"+suffix+".csv",sep=',',header=True, index=False)
            val_chunk.to_csv(output_dir+"val_data"+suffix+".csv",sep=',',header=True, index=False)
            test_chunk.to_csv(output_dir+"test_data"+suffix+".csv",sep=',',header=True, index=False)
        else :
            train_chunk.to_csv(output_dir+"train_data"+suffix+".csv",sep=',',header=False, index=False, mode="a")
            val_chunk.to_csv(output_dir+"val_data"+suffix+".csv",sep=',',header=False, index=False, mode="a")
            test_chunk.to_csv(output_dir+"test_data"+suffix+".csv",sep=',',header=False, index=False, mode="a")
